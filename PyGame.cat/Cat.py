...
color_cat = [254, 144, 45]
color_moustache = [159, 159, 150]
color_eye = [117, 150, 81]
white = [255, 255, 255]
...
def main():
    room(xs, ys)
    window(xs / 1.8, 0.016 * ys, 300, 1.334 * 300)
    boll(497, 845, 80, screen)
    cat(30, 480, 300, color_cat, color_eye, screen)
...
def window(xo, yo, weiht, heg):
    ...
    rect(screen, color_win2, (xo+int(weiht*0.066)*2+int(r), yo+int(heg*0.025*3)+int(heg*0.33), int(r), int(heg*0.33*1.7)))
...
def cat(xc, yc, wegh, colorcat, coloreye, screen1):
    ...
    # тело кот
    ellipse(screen1, color_cat, [xc+wegh*0.37, yc, wegh*1.467, wegh*0.767])
    ellipse(screen1, black, [xc+wegh*0.37, yc, wegh*1.467, wegh*0.767], 1)
    # лапка за головой
    ellipse(screen1, color_cat,  [xc+wegh*0.1367, yc+wegh*0.334, wegh*0.1834, wegh*0.2667])
    ellipse(screen1, black, [xc+wegh*0.1367, yc+wegh*0.334, wegh*0.1834, wegh*0.2667], 1)
    # Голова
    ellipse(screen1, color_cat, (xc, yc, wegh*0.6, 0.533*wegh))
    ellipse(screen1, black, (xc, yc, wegh * 0.6, 0.533*wegh), 1)
    # Ухо левое
    polygon(screen1, color_cat, [(xc - 0.0067*wegh, yc - 0.033*wegh), (xc + 0.166*wegh, yc + 0.05*wegh),
    ...
    # Ухо правое
    polygon(screen1, color_cat, [(xc + wegh * 0.59, yc - 0.0334*wegh),
    ...
    # лапка передняя
    ellipse(screen1, color_cat, [xc+0.3*wegh, yc+0.5167*wegh, 0.3667*wegh, 0.2*wegh])
    ellipse(screen1, black, [xc+0.3*wegh, yc+0.5167*wegh, 0.3667*wegh, 0.2*wegh], 1)
    # лапка задняя
    ellipse(screen1, color_cat,  [xc+1.367*wegh, yc+0.334*wegh, 0.5*wegh, 0.467*wegh])
    ellipse(screen1, black, [xc+1.367*wegh, yc+0.334*wegh, 0.5*wegh, 0.467*wegh], 1)
    ellipse(screen1, color_cat, [xc+1.767*wegh, yc+0.6*wegh, 0.167*wegh, 0.3667*wegh])
